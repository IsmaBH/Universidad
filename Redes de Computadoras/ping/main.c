...
main ()
{
  ...
  if (packet_socket == -1)
  ...
  else
    {
      ...
      while(i < 20)
       {
         solicitudEco(bandera, packet_socket, indice, identificador1, identificador2, secuencia, contador);
         secuencia[0] = secuencia[0] + 1;
         recibeTrama(packet_socket, tramaRev2, identificador1, identificador2);
         contador++;
         i++;

...
{  
  ...
  recibeTrama(ds, tramaRev, "/0x00/", "/0x00");
}

void recibeTrama (int ds, unsigned char *trama,unsigned char *identificador1, unsigned char *identificador2)
{
    unsigned char OP2[2] = { 0x00, 0x02 };
    unsigned char secuencia[2];
    unsigned char ttl[1];
    int bandera = 0;
    int tam, i, len;
	...
	}else
            {
              while (mtime < 25)
	            ...
	            tam = recvfrom (ds, trama, 1514,MSG_DONTWAIT, NULL, 0);
                    if(!memcmp (MACorigen, trama + 0,6) && !memcmp(identificador1, trama + 38,1) && !memcmp(identificador2, trama + 39,1))
                     { 
                       memcpy(secuencia, trama + 40, 2);
                       memcpy(ttl, trama + 22, 1);
                       printf("recibi la respuesta de ECO con el numero de secuencia = %hhu, ttl = %hhu \n", secuencia[1], *ttl);
                       bandera = 1;
                     }
...
void
solicitudEco (int bandera, int ds, int index, unsigned char *identificador1, unsigned char *identificador2, unsigned char *secuencia, int contador)
{
  int cantidad;

...
elemento Dequeue(cola * c)
{
	...
	else
	{
		...
		//El nuevo frente de la cola es el siguiente de e
		c->frente = e.siguiente;
		
		//Si el nuevo frente es NULL, la cola ha quedado vacia y se inicializa
		if(c->frente == NULL) 
		{
			c->final = NULL;
		}
...
boolean Empty(cola * c)
{
	return (c->frente == NULL && c->final == NULL) ? TRUE : FALSE;
}
...
void Destroy(cola * c)
{
	elemento * aux;	//Apuntador auxiliar a elemento	
